...
{{-- car approver list of request waiting for approval --}}
@if($car_approves->count()!=0)
<div class="m-3 text-xl">
    Senarai Permohonan Yang Memerlukan Penilaian Anda
</div>
<div class="m-3">


    <?php
    foreach ($car_approves as $car_approve)
    {
        $car_approve_array[]=$car_approve->id;
    }
    ?>
    ...
    <table class="border-collapse border border-green-900 w-full">
        <thead>
            <tr class="bg-green-700 text-white p-3 rounded">
                ...
                <th width="15%" class="border border-blue-900">Nama Pemohon</th>
        ...
        <tbody>
            ...
            @if(in_array($car_request->car_id,$car_approve_array))
            ...
            ?>
                ...
                <td class="border border-gray-300 p-3 px-5">
                        ...
                        <form action="/cars/request/request-edit/{{$car_request->id}}" method="GET">
                            @csrf
                            <button type="submit" class="bg-yellow-500 hover:bg-yellow-300 text-gray-800 font-bold py-2 px-4 rounded"> 
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
@endif
{{-- END for -> car approver list of request waiting for approval --}}

<div class="m-3 text-xl">
    Senarai Permohonan Anda
</div>
<div class="m-3">
    <a href="{{route('cars-request-request-use')}}" class="bg-green-700 hover:bg-green-500 text-white p-2 rounded">&#43; Mohon Penggunaan Kenderaan Baharu</a>
</div>

@if (session('status'))
    <div class="bg-red-300 text-red-900 p-2 rounded shadow m-3" id="status_message">
        {{ session('status') }}
    </div>
@endif
{{-- 
